...
function reject(reason: string | Error): Promise<never> {
    if (isString(reason)) return Promise.reject(new Error(reason));
...
function isValidCredentials(credentials: Credentials): boolean {
    ...
    const specialCharacters: string[] = password
        .split('')
...
async function register(credentials: Credentials, role: 'client' | 'vendeur'): Promise<IUser> {
    const exists = await userExists(credentials.email);

    if (exists) return reject('User with this email already exists');
    if (!isValidCredentials(credentials)) return reject('Invalid credentials');
    return saveUser({
...
async function login(credentials: Credentials): Promise<string> {
    return retrieveUser(credentials.email)
        .then(async (user: IUser) => {
            ...
            if (!validCredentials) return reject('Invalid credentials');
